...
@pytest.mark.parametrize(
    "figure_1, figure_2, expected_result",
...
def test_add_aria_positive(figure_1, figure_2, expected_result):
    assert figure_1.add_area(figure_2) == expected_result, f"Ожидали {expected_result}"
...
@pytest.mark.parametrize("non_figure", [-3, 42, "abc"])
...
@pytest.mark.parametrize(
    ("side_a", "aria", "perimeter"), [(3, 28.274333882308138, 18.84955592153876)]
)
...
@pytest.mark.parametrize(
    ("side_a", "site_b", "side_c", "aria", "perimeter"), [(13, 14, 15, 84, 42)]
)
def test_triangle_positive(side_a, site_b, side_c, aria, perimeter):
    s = Triangle(side_a, site_b, side_c)
    assert s.get_area() == aria, (
        f"Площадь треугольника со сторонами {side_a}, {site_b} и {side_c}"
    )
